<template>
    <v-app-bar app color="indigo" dense dark>
        ...
        <template v-if="$root.isLogged">
            <a @click.prevent="dialog = true" v-if="!$root.user.isLawyer()" class="text-decoration-none white--text mr-5" to="/appointments">Book Now</a>

            <a @click.prevent="dialogAsLawyer = true" v-if="$root.user.isLawyer()" class="text-decoration-none white--text mr-5" to="/appointments">Make Appointment</a>
